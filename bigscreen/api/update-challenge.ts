...
export default async function handler(req: any, res: any) {
  ...
  if (!challengeId || !status) return res.status(400).json({ error: 'missing_params' })

  const { data, error } = await supabase.from('challenges').update({ status }).eq('id', challengeId).select()
  if (error) return res.status(500).json({ error: 'update_failed', details: error.message })
  return res.status(200).json({ ok: true, challenge: data?.[0] })
}
