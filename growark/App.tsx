...
function App() {
  ...
  const [classes, setClasses] = useState<string[]>(['黄老师班级','姜老师班级','龙老师班级']);
  const [identity, setIdentity] = useState<'teacher'|'principal'>('teacher');
  const [teacherClass, setTeacherClass] = useState<string>('黄老师班级');
  ...
  const handleUpdateScore = async (ids: string[], points: number, reason: string, exp?: number) => {
    ...
    await apiUpdateScore(ids, points, reason, additionalExp);
  };

  const handleHabitCheckIn = async (studentIds: string[], habitId: string) => {
      await handleUpdateScore(studentIds, 5, '习惯打卡');
      await apiHabitCheckIn(studentIds, habitId);
  ...
  const handleChallengeStatus = async (id: string, result: 'success' | 'fail') => {
      ...
      const chNow = challenges.find(c => c.id === id)
      if (result === 'success' && chNow && chNow.status === 'active') {
        await handleUpdateScore(chNow.participants, chNow.rewardPoints, `挑战成功: ${chNow.title}`, chNow.rewardExp)
      }
      ...
      await apiChallengeStatus(id, result, { participants: ch?.participants, title: ch?.title })
  };

  // New Handler: Update PK Winner
  const handlePKWinner = async (id: string, winnerId: string) => {
      const target = pkMatches.find(p => p.id === id)
      if (target) {
        const loserId = target.studentA === winnerId ? target.studentB : target.studentA
        await handleUpdateScore([winnerId], 20, `PK获胜: ${target.topic}`)
        await handleUpdateScore([loserId], 5, `PK参与: ${target.topic}`)
      }
      setPkMatches(prev => prev.map(pk => pk.id === id ? ({ ...pk, status: 'finished', winnerId }) : pk))
      ...
      await apiPKWinner(id, winnerId, { topic: pk?.topic })
  };
  ...
  const handleCompleteTask = async (id: string) => {
      ...
      await apiCompleteTask(id, { title: task.title, expValue: task.expValue })
  };
  ...
  const handleBadgeGrant = async (badgeId: string, studentId: string) => {
      ...
      if (badge) {
          await handleUpdateScore([studentId], 50, `获得勋章: ${badge.name}`, 200);
          const date = new Date().toISOString();
          setStudents(prev => prev.map(s => {
            if (s.id === studentId) {
          ...
          await apiGrantBadge(badgeId, studentId, { name: badge.name })
      }
  };
